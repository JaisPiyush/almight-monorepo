...
library Tick {
    ...
    /// @return feeGrowthInside0X128 The all-time fee growth in token0, 
    /// per unit of liquidity, inside the position's tick boundaries
    /// @return feeGrowthInside1X128 The all-time fee growth in token1, 
    /// per unit of liquidity, inside the position's tick boundaries
    ...
    /// @return liquidityNet The amount of liquidity added (subtracted) 
    /// when tick is crossed from left to right (right to left)
    function cross(
        mapping(int24 => Tick.Info) storage self,
        int24 tick,
